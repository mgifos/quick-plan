...
case class DistanceDuration(distance: Float, unit: DistanceUnit) extends Duration {
  override def json: Json =
    Json.obj(
      "endCondition"              -> Json.obj("conditionTypeKey" -> Json.fromString("distance"), "conditionTypeId" -> Json.fromInt(3)),
...
case class TimeDuration(minutes: Int = 0, seconds: Int = 0) extends Duration {
  override def json: Json =
    Json.obj(
      "endCondition"              -> Json.obj("conditionTypeKey" -> Json.fromString("time"), "conditionTypeId" -> Json.fromInt(2)),
...
object LapButtonPressed extends Duration {
  override def json: Json =
    Json.obj(
      "endCondition"              -> Json.obj("conditionTypeKey" -> Json.fromString("lap.button"), "conditionTypeId" -> Json.fromInt(1)),
      "preferredEndConditionUnit" -> Json.Null,
