...
/**
 * Handles authentication against Garmin Connect via a 6-step OAuth1→OAuth2 flow. Caches the
 * resulting [[GarminSession]] and guards concurrent login attempts with a mutex.
 */
class GarminAuth(
  email: String,
  password: String,
...
/** Factory for [[GarminAuth]]. */
object GarminAuth {
  def make(email: String, password: String, client: Client[IO]): IO[GarminAuth] =
    for {
      ref <- Ref.of[IO, Option[GarminSession]](None)
      mutex <- Mutex[IO]
    } yield new GarminAuth(email, password, client, ref, mutex)
}
